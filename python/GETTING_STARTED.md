# Getting Started with Roman Numerals Converter kata in Python
...
```shell
cd Kata-RomanNumeralsConverter/python
```
...
> ***Reminder***: the commands below should be run from [Kata-RomanNumeralsConverter/python](.) directory
...
`File` > `Open` > `Kata-RomanNumeralsConverter` > `python`
...
> ***Reminder***: the command below should be run from [Kata-RomanNumeralsConverter/python](.) directory
...
Choose `Open Folder`, navigate to `Kata-RomanNumeralsConverter` / `python` then click `Select Folder`.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-RomanNumeralsConverter` / `python`
