# Getting Started with Roman Numerals Converter kata in C++
...
- Create a build directory: ***Kata-RomanNumeralsConverter/cpp/build***. All build-related files are generated under this directory.
...
- Generate the solution file ***Kata-RomanNumeralsConverter.sln*** for **Visual Studio 2022** on Windows,
...
> ***Reminder***: the commands below should be run from [Kata-RomanNumeralsConverter/cpp]() directory
...
> ***Reminder***: the command below should be run from [Kata-RomanNumeralsConverter/cpp]() directory
...
1. Make sure the 'Kata-RomanNumeralsConverter-test' project is set as the Start-up Project.
...
`File` > `Open` > `Kata-RomanNumeralsConverter` > `cpp`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-RomanNumeralsConverter` / `cpp`
...
Open Xcode, choose `Open a project or file`, navigate to `Kata-RomanNumeralsConverter` / `cpp` / `build` / `Kata-RomanNumeralsConverter.xcodeproj`
then click `Open`.

Once the project is opened in Xcode, select `Product` / `Scheme` / `Kata-RomanNumeralsConverter-test` to set it as the current active scheme.
