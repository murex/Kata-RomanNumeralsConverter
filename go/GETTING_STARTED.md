# Getting Started with Roman Numerals Converter kata in Go
...
```shell
cd Kata-RomanNumeralsConverter/go
```
...
> ***Reminder***: the commands below should be run from [Kata-RomanNumeralsConverter/go](.) directory
...
> ***Reminder***: the command below should be run from [Kata-RomanNumeralsConverter/go](.) directory
...
`File` > `Open` > `Kata-RomanNumeralsConverter` > `go`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-RomanNumeralsConverter` / `go`
