...
`File` > `Open Folder` > `Kata-RomanNumeralsConverter` > `java`
...
`File` > `Open Projects from File System...` > `Kata-RomanNumeralsConverter`
...
3. From the terminal navigate to [Kata-RomanNumeralsConverter/java]() directory
